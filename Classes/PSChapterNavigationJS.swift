...
//
//  === Two emission sites, and why (the jerky-jump fix) ===
//
//  This enum now emits **two** blocks, and where each one lands in the document is
//  load-bearing:
//
//   * `script(entryCount:)` → `<head>`, via `createHTMLString`'s `withJS:`. Function
//     definitions plus a `window.onload` that re-measures and re-applies.
//   * `bootScript(extraJS:)` → the very END of `<body>`, appended by
//     `PSContentReader.chapterPage` after the six pads. This runs after parse but
//     BEFORE the first paint, so the first frame is already at the target verse.
//
//  `extraJS` — the app's `scrollToVerse(n);` / `scrollToPosition(y);` call — used to
//  sit inside `window.onload` alongside `resetArrays()`. `onload` fires *after* the
//  first paint, so the user saw the top of the chapter painted and only then a jump;
//  and `scrollToVerse` / `scrollToPosition` each wrapped their work in a further
//  250 ms `setTimeout`, which is where the rest of the visible pause came from. Both
//  are gone: the timeouts are unwrapped and the call moved to `bootScript`.
//
//  `_scrollToVerse` is split so this can be done safely. It records the request in
//  `psScrollRequest` and delegates the actual move to `__psScrollToVerseNow`;
//  `psReapplyInitialScroll()` in `onload` re-runs it once against freshly-measured
//  offsets, because anything that loads after parse (a webfont, an image) can shift
//  them.
//
//  That re-apply is **guarded** on `psUserScrolled`, set by capturing `touchstart` /
//  `wheel` listeners, so a user who has already started scrolling is never yanked
//  back. The guard deliberately keys on *input*, not on offset: comparing
//  `window.pageYOffset` against the value applied at boot cannot tell a user scroll
//  from a reflow, and a reflow is precisely the case the re-apply exists to fix — it
//  would disable itself exactly when needed. `capture: true` matters too, since page
//  content could otherwise stop the event before it reaches the document.
//
...
enum PSChapterNavigationJS {
    ...
    /// The `<head>` `<script>` block for a chapter with `entryCount` entries:
    /// the function definitions plus the `window.onload` re-measure/re-apply. The
    /// initial scroll itself is issued by `bootScript(extraJS:)` at the end of
    /// `<body>` — see this file's header for why the split exists.
    ///
    /// - Parameter entryCount: `-chapterBodyHTML:`'s loop counter — NOT a verse
    ///   count. It bounds the `versepos` array and both scan loops, and matches the
    ///   `vv{i}` anchor numbering the assembler emitted.
    static func script(entryCount: Int) -> String {
