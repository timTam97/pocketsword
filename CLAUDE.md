...
A standalone macOS CLI (plain `Makefile` + `main.mm`, deliberately **not** an Xcode target) that links the same vendored engine and bakes the five bundled modules into `Resources/PSContent.sqlite` + `Resources/Versification-KJV.json`. It exists for the SWORD-removal work (`SWORD_REMOVAL_PLAN.md` Phase 2) and is deleted in Phase 5 with `externals/sword`. Nothing in the app reads its output yet, and **the artifacts are intentionally absent from the Copy Resources phase** — don't "fix" that until Phase 3 cuts the reader over.
...
- `tools/swordbake/README.md` records the measurements and the engine quirks the store had to be designed around (two encodings, the `getChapter` loop counter, the headings axis, the fifth `passagestudy.jsp` emission site). Read it before touching the schema.
