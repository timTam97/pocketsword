...
5. **The instance cache closes the handle before evicting**, or it leaks with the last reference. Note `engine(forModuleName:)` is spelled as a factory because it returns a *shared* instance; the Obj-C `+engineForModuleName:` imported into Swift as `init(forModuleName:)`, which read like it made a new one and did not.
