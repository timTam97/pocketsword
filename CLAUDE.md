...
The app is a **mixed Swift / Objective-C++ target**. The app layer under `Classes/` was migrated to Swift (see `SWIFT_MIGRATION_PLAN.md`); the only remaining Objective-C(++) is the **SWORD bridge** (`Classes/Sword*.{h,mm}`, `PSSearchEngine.{h,mm}`, `VerseEnumerator.{h,mm}`) plus one clean DTO (`SwordModuleTextEntry.m`). That bridge is on its way out: `SWORD_REMOVAL_PLAN.md` Phase 3 (done) moved content *reading* to a pure-Swift reader over a baked SQLite store, and Phase 5 deletes the engine and the bridge entirely. **Swift never touches C++** — it talks to SWORD only through the Foundation-only Obj-C facades. There is no CocoaPods/SPM; all third-party code is vendored under `externals/`. The UI is programmatic UIKit — there are **no XIBs** and only a `LaunchScreen.storyboard`.
...
### `tools/swordbake` — the offline content converter (dev-only, not part of the app build)

A standalone macOS CLI (plain `Makefile` + `main.mm`, deliberately **not** an Xcode target) that links the same vendored engine and bakes the five bundled modules into `Resources/PSContent.sqlite` + `Resources/Versification-KJV.json`. It exists for the SWORD-removal work (`SWORD_REMOVAL_PLAN.md`) and is deleted in Phase 5 with `externals/sword`. Both artifacts **are** bundled and **are** the app's render path as of Phase 3 (they are bundled by path, so a re-bake needs no pbxproj change). The store is at **schemaVersion 2 / tokenGrammar v2**, and `Classes/PSContentStore.swift` refuses to open anything else — the writer and the reader must be changed together.

- `make -C tools/swordbake run` to regenerate, `make verify` to prove byte-stability, `python3 tools/swordbake/crosscheck.py` to diff the store against the committed live-SWORD fixtures.
- `tools/swordbake/sword_sources.txt` lists the 148 engine translation units, extracted from the app's own Sources build phase. If you add or remove a SWORD source in the pbxproj, update that file too or the tool silently compiles a different engine than the app.
- `ftplib.c` / `ftpparse.c` are **C**, not C++ — the Makefile compiles them with `clang`, and the app's build phase likewise treats them as C.
- `externals/sword/include/zlib.h` is zlib **1.1.4** and shadows the SDK header on any TU that has `externals/sword/include` on its include path. It predates `compressBound`. The app never noticed because it only calls `compress2`/`uncompress`, whose signatures are unchanged.
- `tools/swordbake/README.md` records the measurements and the engine quirks the store had to be designed around (two encodings, the `getChapter` loop counter, the headings axis, the fifth `passagestudy.jsp` emission site, and why the FTS text is captured with four options **off**). Read it before touching the schema.
...
**As of Phase 3 the app reads content from the baked SQLite store, not from SWORD.**
`PSFeatureFlags.swiftContentReader` is **default-on**; the engine is still in the tree
underneath as a fallback and as the differential oracle, and goes in Phase 5.
...
**Failure policy.** Every failure returns nil after reporting through
`PSContentStore.fail` (assertionFailure in debug, `alog` in release), so the caller
falls back to SWORD — a blank chapter must never be the user-visible outcome of a
reader bug. `PSContentReader.swift`'s header lists all ten such conditions, and that
list is exactly what **Phase 5 must convert into hard failures** once there is no
...
- The chapter-navigation JS lives in **one** place,
  ...
  copy it into Swift, move it in Phase 5.
